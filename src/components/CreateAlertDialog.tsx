...
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from './design-system/Dialog';
...
export function CreateAlertDialog({ open, onOpenChange, onSave, editingAlert, currentUserName }: CreateAlertDialogProps) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      siteIds,
      jobFunctionTypes,
      taskIds: [], // For simplicity, not implementing task-level filtering in UI
      recipients,
  ...
  const getSeverityIcon = () => {
    switch (severity) {
      ...
      case 'warning':
        return <AlertTriangle style={{ width: '16px', height: '16px', color: '#eab308' }} />;
      case 'info':
        return <Info style={{ width: '16px', height: '16px', color: 'var(--chart-1)' }} />;
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="dialog-content-medium">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure automatic alerts to notify you when performance metrics cross thresholds.
          </DialogDescription>
        </DialogHeader>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-6)', padding: 'var(--spacing-4) 0' }}>
          {/* Basic Info */}
